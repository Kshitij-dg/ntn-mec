"""
train_ctde.py
Train all UE agents with Centralized Training / Decentralized Execution (CTDE).

Usage:
    python train_ctde.py [--episodes 10000] [--steps 50] [--alpha 0.2] [--seed 42]
"""
...
from agents.trainer import run_episode_ctde, evaluate
...
def parse_args():
    ...
    p.add_argument('--alpha',     type=float, default=0.5)   # meaningful global weight
    p.add_argument('--seed',      type=int,   default=42)
    p.add_argument('--log_every', type=int,   default=200)
    p.add_argument('--eval_every',type=int,   default=1000)
    p.add_argument('--save_dir',  type=str,   default='checkpoints/ctde')
    return p.parse_args()
...
def main():
    ...
    for ep in range(1, args.episodes + 1):
        result = run_episode_ctde(env, agents, steps_per_episode=args.steps, train=True)
        log.append(result)
        ...
        if ep % args.log_every == 0:
            ...
            print(f"[CTDE] ep={ep:5d}  reward={mean_r:8.2f}  "
    ...
    with open(os.path.join(args.save_dir, 'log_ctde.json'), 'w') as f:
        json.dump([{k: v for k, v in r.items()} for r in log], f, indent=2)

    print("CTDE training complete.")
